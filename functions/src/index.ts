  ...
  async (request) => {
    ...
    if (!keyId || !keySecret) {
      throw new HttpsError(
        'failed-precondition',
        'Razorpay keys are not configured. Set the RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET secrets and redeploy the functions.'
      );
    }
    ...
    try {
      ...
      razorpayOrder = (await razorpayResponse.json()) as { id: string };
    } catch (err) {
